...
import { mutate, type UpdateUserInput, type User } from '~/graphql'
import { UPDATE_ACCOUNT_MUTATION } from '~/graphql/queries'

export const updateAccountAction: ActionFunction = async ({ request }) => {
  ...
  const data = Object.fromEntries(formData) as UpdateUserInput
