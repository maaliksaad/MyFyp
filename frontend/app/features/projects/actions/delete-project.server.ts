import { type ActionFunction, json, redirect } from '@remix-run/node'

import { retrieveUserFromSession } from '~/features/user-authentication/session'
import { mutate, type Project } from '~/graphql'
import { DELETE_PROJECT_MUTATION } from '~/graphql/queries'

export const deleteProjectAction: ActionFunction = async ({ request }) => {
  const user = await retrieveUserFromSession(request)

  if (user == null) {
    return redirect('/login')
  }

  const formData = await request.formData()
  const data = Object.fromEntries(formData) as { project_id: string }

  const response = await mutate<{ delete_project: Project }>({
    mutation: DELETE_PROJECT_MUTATION,
    variables: {
      id: +data.project_id
    },
    headers: {
      Authorization: `Bearer ${user.token}`
    }
  })

  if (response.errors != null) {
    return json({ errors: response.errors }, { status: 400 })
  }

  if (response.data?.delete_project == null) {
    return json({ errors: { message: 'Unexpected Error' } }, { status: 400 })
  }

  return redirect(`/projects`)
}
