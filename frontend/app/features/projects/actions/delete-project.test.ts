...
describe('Delete Project Action', () => {
  test('given a logged out user: should redirect to the /login', async () => {
    const request = new Request(
      'http://localhost:3000/projects/my-project/delete'
    )
  ...
  test('given an error while deleting project: should return a 400 status', async () => {
    const request = await createAuthenticatedRequest({
      url: 'http://localhost:3000/projects/my-project/delete',
      formData: toFormData({
