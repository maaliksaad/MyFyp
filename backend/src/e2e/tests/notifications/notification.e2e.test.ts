...
describe('Query: notifications', () => {
  ...
  it('given a logged in user with notifications: should return user notifications', async () => {
    const user = createPopulatedUser({
      verified: true
    })
    await userModel.create({ ...user })

    const notification = createPopulatedNotification({ user_id: user.user_id })
